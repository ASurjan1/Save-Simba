...
void delay100ms(uint16_t delayTime){
	delayFlag = 0;
	for (int i = 0; i < (100 * delayTime); i++){
		TIMER1_CTL_R |= TIMER_CTL_TAEN;
    while (!delayFlag) {}
    delayFlag = 0;
    TIMER1_CTL_R &= ~TIMER_CTL_TAEN; // Disable Timer1A interrupt
	}
}

void delay10ms(uint16_t delayTime){
	delayFlag = 0;
	for (int i = 0; i < (10 * delayTime); i++){
		TIMER1_CTL_R |= TIMER_CTL_TAEN;
    while (!delayFlag) {}
    delayFlag = 0;
    TIMER1_CTL_R &= ~TIMER_CTL_TAEN; // Disable Timer1A interrupt
